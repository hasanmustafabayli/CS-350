...
void queue_init(struct queue * the_queue, size_t queue_size)
{
    ...
    the_queue->queue_limit = queue_size;
    the_queue->size = 0; 
    the_queue->front = 0;
    the_queue->rear = -1;
}
...
int add_to_queue(struct request_meta to_add, struct queue * the_queue)
{
	...
	if (the_queue->size >= the_queue->queue_limit) {
		/* What to do in case of a full queue */
		retval = 1;
	} else {
		/* If all good, add the item in the queue */
		the_queue->rear = (the_queue->rear + 1) % the_queue->queue_limit;
...
struct request_meta get_from_queue(struct queue * the_queue)
{
	...
	if (the_queue->size > 0) {
        ...
        the_queue->front = (the_queue->front + 1) % the_queue->queue_limit;
        the_queue->size--;
    }
    else {
...
void dump_queue_status(struct queue * the_queue)
{
	...
	for (int i = 0; i < the_queue->size; i++) {
		uint64_t current_id = the_queue->items[(the_queue->front + i) % the_queue->queue_limit].request.req_id;
...
{
	struct timespec now, completion_timestamp, start_timestamp;
	...
	while (!params->worker_done) {
		...
		cr.req_id = req.request.req_id;
		cr.ack = 0;
		
		size_t bytes_sent = send(conn_socket, &cr, sizeof(struct response), 0);
		if (bytes_sent == -1) {
		...
		printf("R%ld:%lf,%lf,%lf,%lf,%lf\n", req.request.req_id, sent_timestamp, length, receipt_time, start_time, completion_time);
...
{
    int cp = clone(worker_main, (char*)worker_stack + STACK_SIZE,  CLONE_THREAD | CLONE_VM | CLONE_SIGHAND | CLONE_FS | CLONE_FILES | CLONE_SYSVSEM, params);
    return cp;
}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	do {
		...
		if (add_to_queue(*req, the_queue) == 1) { 
			cr.req_id = req->request.req_id;  
			cr.ack = 1;
			double sent=TSPEC_TO_DOUBLE(req->request.req_timestamp);
			double length=TSPEC_TO_DOUBLE(req->request.req_length);
			double reject=TSPEC_TO_DOUBLE(receipt_timestamp);
			printf("X%lu:%lf,%lf,%lf\n", cr.req_id, sent, length, reject);
			send(conn_socket, &cr, sizeof(struct response), 0);
		}
	} while (in_bytes > 0);
