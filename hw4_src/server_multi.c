...
int add_to_queue(struct request_meta to_add, struct queue * the_queue)
{
	...
	if (the_queue->size == the_queue->max) {
		...
		the_queue->rear = (the_queue->rear + 1) % the_queue->max;
...
struct request_meta get_from_queue(struct queue * the_queue)
{
	...
	if (the_queue->size > 0) {
        ...
        the_queue->front = (the_queue->front + 1) % the_queue->max;
        the_queue->size--;
    }
...
void dump_queue_status(struct queue * the_queue)
{
    ...
    for (int i = 0; i < the_queue->size; i++) {
		uint64_t id = the_queue->items[(the_queue->front + i) % the_queue->max].request.req_id;
        sync_printf("R%lu", id);
...
int worker_main (void * arg)
{
	...
	while (!params->worker_done) {
		...
		size_t sent = send(conn_socket, &resp, sizeof(struct response), 0);
		if (sent== -1) {
			perror("Fail to send response \n");
			break;
		}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	struct worker_params * worker_params = malloc(sizeof(struct worker_params) * conn_params.servers);
	void** worker_stacks = malloc(conn_params.servers * sizeof(void *));
	
	for (int i = 0; i < conn_params.servers; i++) {
		worker_stacks[i] = malloc(STACK_SIZE);
		if(worker_stacks == NULL){
			perror("No allocated memory space.\n");
      		exit(EXIT_FAILURE);
		}
		if (worker_id < 0) {
...
int main (int argc, char ** argv) {
	int sockfd, retval, accepted, optval, opt, queue_size, servers;
