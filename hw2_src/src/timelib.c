...
uint64_t get_elapsed_sleep(long sec, long nsec)
{
	...
	sleep_duration.tv_sec=sec;
	sleep_duration.tv_nsec=nsec;
	get_clocks(start);
...
uint64_t get_elapsed_busywait(long sec, long nsec)
{
	...
	timespec_add(&end_timestamp, &begin_timestamp); //Just realized that I have a helper functions :(((
    while (timespec_cmp(&end_timestamp, &begin_timestamp)>0) {
        clock_gettime(CLOCK_MONOTONIC, &begin_timestamp);
	}
	get_clocks(end);
    uint64_t elapsed_cycles = end - start;
